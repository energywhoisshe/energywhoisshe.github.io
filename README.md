...
`<3 Households 집을 지키는 사람들> 프로젝트는 아트센터나비에서 게임으로 개발되었고, 웹진세미나에서 기사로 발행되며, 부천아트벙커에서 미디어파사드로 설치된다. 세 개의 시리즈는 모두 유니티 언어로 기술되었다.`

> `이 글은 작업이 공간에 설치되기 약 1~3달 전에 쓰여집니다. PC에 최적화된 문서로, 모바일 기기는 스크롤을 상하좌우로 움직일 필요가 있습니다. 자동 재생이 되지 않았다면 플레이 버튼을 클릭해주세요. 작고 자라서 얼마간 큰 소리가 포함되어있으니 볼륨 조절에 유의하십시오.`
